      ...
      <div class="task-list-container">
        ...
        >
          <template #updated_at="{ record }">
            {{ dateStrToYMD(record.updatedAt) }}
          </template>
          <template #created_at="{ record }">
            {{ dateStrToYMD(record.createdCt) }}
          </template>
          ...
          <template #operations="{ record }">
            <ConfigCenterOperation
                @onDeleted="fetchData"
                config="ai" :config-id="record.id" edit-route="editAiConfig"></ConfigCenterOperation>
          </template>
        </a-table>
      </div>
    </a-card>
...
import {deleteConfig, queryConfigList, toggleConfigIsEnable} from '@/api/config';
...
const fetchData = async (
  ...
  setLoading(true);
  try {
    const {data} = await queryConfigList('ai', params);
...
const handlerIsEnableChange = async (checked: number, record: any) => {
  console.log(record);
  await toggleConfigIsEnable("ai", {
    id: record.id,
    enabled: checked,
  });
};
const redirectToOtherPage = () => {
  router.push({name: 'createAiConfig'});
};
